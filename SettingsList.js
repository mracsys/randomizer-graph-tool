...
for (var line of lines) {
    ...
    if (parsing) {
        if (['setting_info(', 'checkbutton(', 'combobox(', 'scale('].some((prefix) => line.toLowerCase().startsWith(prefix))) {
            split_char = '=';
            if (setting.name !== '' && !(setting.name in settings_list)) {
                if (setting.default === null) {
                    switch(setting.type) {
                        case 'bool':
                            settings_list[setting.name] = false;
                            break;
                        case 'str':
                            settings_list[setting.name] = '';
                            break;
                        case 'int':
                            settings_list[setting.name] = 0;
                            break;
                        case 'list':
                            settings_list[setting.name] = [];
                            break;
                    ...
                    settings_list[setting.name] = setting.default;
                }
            }
            arg_test = line.split('(');
